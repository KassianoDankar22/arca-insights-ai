...
-- Tabela de transações financeiras (sem referência a property_id por enquanto)
CREATE TABLE IF NOT EXISTS transactions (
    ...
    property_reference VARCHAR(255), -- Referência textual por enquanto
    metadata JSONB,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
...
-- Triggers para updated_at (apenas para tabelas que têm updated_at)
DO $$
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'chat_conversations' AND column_name = 'updated_at') THEN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'roi_analyses' AND column_name = 'updated_at') THEN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'course_progress' AND column_name = 'updated_at') THEN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'crm_leads' AND column_name = 'updated_at') THEN
...
-- Adicionar property_id à tabela transactions após criação da tabela properties
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'properties') 
       AND NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'transactions' AND column_name = 'property_id') THEN
