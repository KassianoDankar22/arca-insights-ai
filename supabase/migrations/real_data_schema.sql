...
-- Criação de todas as tabelas necessárias para dados reais
...
CREATE POLICY "Users can create own progress" ON course_progress FOR INSERT WITH CHECK (auth.uid() = user_id);
...
CREATE POLICY "Users can view messages from own tickets" ON ticket_messages FOR SELECT USING (
    ticket_id IN (SELECT id FROM support_tickets WHERE user_id = auth.uid())
);
CREATE POLICY "Users can insert messages to own tickets" ON ticket_messages FOR INSERT WITH CHECK (
    ticket_id IN (SELECT id FROM support_tickets WHERE user_id = auth.uid())
);

-- Políticas RLS para live_chat_sessions
CREATE POLICY "Users can view own chat sessions" ON live_chat_sessions FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can create own chat sessions" ON live_chat_sessions FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update own chat sessions" ON live_chat_sessions FOR UPDATE USING (auth.uid() = user_id);

-- Políticas RLS para live_chat_messages
CREATE POLICY "Users can view messages from own sessions" ON live_chat_messages FOR SELECT USING (
    session_id IN (SELECT id FROM live_chat_sessions WHERE user_id = auth.uid())
);
CREATE POLICY "Users can insert messages to own sessions" ON live_chat_messages FOR INSERT WITH CHECK (
    session_id IN (SELECT id FROM live_chat_sessions WHERE user_id = auth.uid())
);
...
COMMENT ON TABLE ticket_messages IS 'Mensagens dos tickets de suporte';
